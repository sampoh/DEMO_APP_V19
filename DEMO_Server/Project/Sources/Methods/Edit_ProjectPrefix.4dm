...
$JSON:=Replace string:C233($JSON; $prefix.old; $prefix.new)
If (Not:C34($isTest))
...
For ($i; 1; $sizeI)
	...
	If (Not:C34($isTest))
		...
		$CODE:=Replace string:C233($CODE; $prefix.old+"_"; $prefix.new+"_")
		TEXT TO DOCUMENT:C1237($DIR+$FROM; $CODE; "UTF-8")
	End if 
	
	If ($FROM=($prefix.old+"@"))
		$TO:=Replace string:C233($FROM; $prefix.old+"_"; $prefix.new+"_")
		If (Not:C34($isTest))
...
If (Test path name:C476($ROOT)=Is a folder:K24:2)
